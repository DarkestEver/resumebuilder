...
const CreativeTemplate: React.FC<TemplateProps> = ({ profile, customizations }) => {
  ...
  return (
    <div className="w-full bg-white p-10" style={{ fontFamily: 'sans-serif', color: '#000000' }}>
      ...
      <div className="mb-6 pb-4 border-b-2 border-gray-900">
        ...
        {profile.personalInfo?.title && (
          <p className="text-lg mb-2">{profile.personalInfo?.title}</p>
        )}
      ...
      {profile.summary && (
        <div className="mb-6">
          <h2 className="text-lg font-bold mb-2 border-b-2 border-gray-900 pb-1">
        ...
        <div className="mb-6">
          ...
          {profile.education.map((edu: any, idx: number) => (
            <div key={idx} className="mb-3">
              ...
              {edu.year && <p className="text-sm">Graduated: {edu.year}</p>}
            </div>
          ))}
        </div>
      )}
