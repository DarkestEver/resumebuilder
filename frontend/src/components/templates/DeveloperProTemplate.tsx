...
const DeveloperProTemplate: React.FC<TemplateProps> = ({ profile, customizations }) => {
  return (
    <div className="w-full bg-white p-10" style={{ fontFamily: 'Consolas, Monaco, monospace', color: '#000000' }}>
      {/* Header */}
        ...
        <div className="mb-6">
          ...
          {profile.experience.map((exp: any, idx: number) => (
            <div key={idx} className="mb-5">
              ...
              {exp.technologies && (
                <p className="text-xs">
                  <span className="font-semibold">Tech Stack:</span> {Array.isArray(exp.technologies) ? exp.technologies.join(', ') : exp.technologies}
                </p>
              )}
            </div>
          ))}
        ...
        <div className="mb-6">
          ...
          {profile.projects.map((proj: any, idx: number) => (
            <div key={idx} className="mb-4">
              ...
              {proj.github && (
                <p className="text-xs">
                  <span className="font-semibold">GitHub:</span> {proj.github}
                </p>
              )}
            </div>
          ))}
...
export default DeveloperProTemplate;
